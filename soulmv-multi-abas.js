...
// - Abre Painel / Transferência / Aviso / Atendimentos / Ficha / Descrição / Confirmação / Transf. CC / Mov. Atendimento / Alta
// - Botão flutuante com: CONFIGURAÇÕES + seleção CO/NIR/MATE + ABRIR ABAS + SALVAR + X
// - Salva configuração por andar em localStorage ("mv-multi-painel-config")

(function () {
  if (!location.href.includes('/soul-mv')) {
    alert('Este botão só funciona dentro do SoulMV.');
    return;
  }
  if (document.getElementById('mv-multi-painel')) return;

  const d = ms => new Promise(r => setTimeout(r, ms));

  function findMenuAnchor(s, t) {
  ...
  async function ensureLoaded(target, openFn) {
    ...
    if (sawLoading && typeof openFn === 'function') {
      openFn();
    }
  }
  ...
  function applyFloorTemplate(code) {
    ...
    let list = null;
    if (saved.floors && saved.floors[key]) {
      list = saved.floors[key];
  ...
  saveBtn.onclick = () => {
    ...
    try {
      ...
      console.error(e);
      alert('Não foi possível salvar (localStorage).');
    }
  };
